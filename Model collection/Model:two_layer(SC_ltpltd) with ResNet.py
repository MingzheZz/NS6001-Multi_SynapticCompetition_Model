...
def update_w(x, w, limit=False, ltd=True, mature=True, compet=True, rate=0.1):
    #limit=True, ltd=False: sc + ltp   |   limit=False, ltd=True: ltd +ltp  |   limit=False, ltd=False: ltp
    #rate = learning rate
...
def cal_fit_rate(W_E_xy: np.ndarray,W_E_yy:np.ndarray,
    ...
    y_l = y_of_x(new_y,W_E_yy)#(n_l1,n_training)(n_l2,n_l1)=(n_l2,n_training)
...
def local_loop(mnist_data, mnist_label, N_train_sample, N_test, N_y1, N_y2):
    ...
    data_layer = y_of_x(mnist_data_training.transpose(),new_W_E_xy)#(n_l1,n_training)

    W_E_yy = np.zeros((N_y2,data_layer.shape[0]))#(n_l2,n_l1)

    W_E_yy = update_w(data_layer.transpose(),W_E_yy,limit=False, ltd=True)#(n_training,n_l1)(n_l2,n_l1)  ltd
