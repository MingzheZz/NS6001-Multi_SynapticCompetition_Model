...
def update_w_l1(x, w):
    ...
    for idx in range(x.shape[0]):
        ...
        w_local += learning_rate_l1 * delta_w
...
def update_w_l2(x, w):
    ...
    for idx in range(x.shape[0]):
        ...
        w_local += learning_rate_l2 * delta_w
...
def update_w_l_nw(x, w):

    learning_rate_nw = 0.1
    w_local = np.copy(w)
    ...
    for idx in range(x.shape[0]):
        ...
        w_local += learning_rate_nw * delta_w
...
def cal_fit_rate(W_E_xy: np.ndarray,W_E_yy:np.ndarray,
    ...
    new_y = np.vstack((y,data_sample.transpose()))#(n_l1,n_training)+(784,n_training)=(n_l1+784,n_training)

    y_l = y_of_x(new_y,W_E_yy)#(n_l1+784,n_training)(n_l2,n_l1+784)=(n_l2,n_training)
    ...
    y = np.vstack((y,data_test.transpose()))#(n_l1,n_test)+(784,n_test)=(n_l1+784,n_test)
...
def local_loop(mnist_data, mnist_label, N_train_sample, N_test, N_y1, N_y2):
    ...
    W_E_xy = update_w_l1(mnist_data_training, W_E_xy)#(n_training,784)(n_l1,784)

    idtt_m = np.eye(784)#(784,784)

    new_W_E_xy = np.vstack((W_E_xy, idtt_m))#(n_l1+784,784)

    data_layer = y_of_x(mnist_data_training.transpose(),new_W_E_xy)#(784,n_training)(n_l1+784,784)=(n_l1+784,n_training)

    W_E_yy = np.zeros((N_y2,data_layer.shape[0]))#(n_l2,n_l1+784)

    W_E_yy = update_w_l2(data_layer.transpose(),W_E_yy)#(n_training,n_l1+784)(n_l2,n_l1)
...
for N_y1 in np.arange(100, 1001, 100):
    for N_y2 in np.arange(100, 1001, 100):
        for N_train_sample in [100, 500, 1000, 5000, 10000, 50000]:
            ...
            sample_ans = joblib.Parallel(n_jobs=-1)(
                ...
                for idx in range(10))
            data_to_write = [N_y1, N_y2, N_train_sample, np.mean(sample_ans), np.std(sample_ans)]
            data_result.append(data_to_write)
