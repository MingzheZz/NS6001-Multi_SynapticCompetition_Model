...
def cal_fit_rate(W_E_xy: np.ndarray,W_E_yy:np.ndarray,
    ...
    eigenvalues1= np.linalg.eigvals(co1)
    eigenvalues2= np.linalg.eigvals(co2)

    l1_variation = np.sum(eigenvalues1)
    l2_variation = np.sum(eigenvalues2)

    target_value1 = 0.8 * l1_variation
    target_value2 = 0.8 * l2_variation

    cumulative_sum1 = 0
    index1 = 0
    cumulative_sum2 = 0
    index2 = 0
    for i, value in enumerate(eigenvalues1):
        cumulative_sum1 += value
        if cumulative_sum1 >= target_value1:
            index1 = i
            break

    for i, value in enumerate(eigenvalues2):
        cumulative_sum2 += value
        if cumulative_sum2 >= target_value2:
            index2 = i
            break
    ...
    return ans, l1_variation,l2_variation,index1,index2
...
def local_loop(mnist_data, mnist_label, N_train_sample, N_test, N_y1, N_y2):
    ...
    W_E_yy = update_w_l2(data_layer.transpose(),W_E_yy)#(n_training,n_l1)(n_l2,n_l1)
...
for N_y1 in np.arange(100, 1001, 100):
    for N_y2 in np.arange(100, 1001, 100):
        ...
        for i in range(10):
            ...
            l1_v.append(result[i][1])
            l2_v.append(result[i][2])
            eig1.append(result[i][3])
            eig2.append(result[i][4])

        
        print(N_y1,N_y2,np.mean(ans), np.std(ans),np.mean(l1_v),np.std(l1_v),np.mean(l2_v),np.std(l2_v),np.mean(eig1),np.mean(eig2),flush=True)
